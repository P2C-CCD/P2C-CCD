...
from .multi_dense_mesh_contact_pairs import _rename_assets
...
class LargeDenseComplexMeshCasesConfig:
    ...
    run_name: str = "large_dense_complex_mesh_cases_run_id"
    shard_root: str = "src/datasets/training/large_dense_complex_mesh_cases/shards"
    training_output_dir: str = "src/outputs/stpf_training"
    benchmark_output_dir: str = "src/benchmark"


@dataclass(frozen=True, slots=True)
class LargeDenseComplexCaseResult:
    case_name: str
...
@dataclass(frozen=True, slots=True)
class LargeDenseComplexMeshCasesResult:
    config: LargeDenseComplexMeshCasesConfig
    ...
    case_results: tuple[LargeDenseComplexCaseResult, ...]
    report_path: Path
    summary_json_path: Path


def _make_heavy_intra_pairs(
    case_name: str,
    assets: Sequence[MeshDensityAsset],
    *,
    limit: int,
) -> tuple[MeshDensityPair, ...]:
    renamed = _rename_assets(assets, case_name)
    pairs = [
        MeshDensityPair(
        ...
        for asset_a, asset_b in combinations(renamed, 2)
    ]
...
def _split_pairs(
    ...
    *,
    ...
    if len(shuffled) < 2:
        raise ValueError("each large dense case needs at least two pairs")
...
def _unique_face_counts(pairs: Sequence[MeshDensityPair]) -> tuple[int, ...]:
    ...
    return tuple(sorted(asset.face_count for asset in by_path.values()))
...
def run_large_dense_complex_mesh_cases_benchmark(
    config: LargeDenseComplexMeshCasesConfig | None = None,
) -> LargeDenseComplexMeshCasesResult:
    cfg = config or LargeDenseComplexMeshCasesConfig()
    ...
    if len(abc) < 2 or len(fusion) < 2 or len(thingi) < 2:
        raise ValueError("not enough assets to construct the three large dense cases")
    ...
    train_dataset, train_case_by_query_id, train_costs = _build_dataset_from_cases(
        train_pairs_by_case,
        first_sample_id=12_000_001,
        samples_per_pair=cfg.samples_per_pair,
    )
    eval_dataset, eval_case_by_query_id, eval_costs = _build_dataset_from_cases(
        eval_pairs_by_case,
    ...
    write_npz_shard(
        ...
        train_dataset,
        metadata={
            **default_metadata(train_dataset, seed=cfg.seed, source="large_dense_complex_mesh_cases"),
            "dataset_role": "train",
        },
    )
    write_npz_shard(
        ...
        eval_dataset,
        metadata={
            **default_metadata(eval_dataset, seed=cfg.seed + 1, source="large_dense_complex_mesh_cases"),
            "dataset_role": "eval",
        },
    )
    ...
    combined_trained_stpf = benchmark_stpf_on_high_density_workload(
        ...
        method_name="LargeDenseComplex-RTSTPFExact-Trained",
    )

    case_results: list[LargeDenseComplexCaseResult] = []
    ...
    result = LargeDenseComplexMeshCasesResult(
        config=cfg,
...
def _write_summary_json(path: Path, result: LargeDenseComplexMeshCasesResult, shard_dir: Path) -> None:
    payload = {
        ...
        "case_results": [
            {
                ...
                "trained_exact_work_reduction_vs_no_proposal": _reduction(item.trained_stpf, item.no_proposal),
            }
...
def _write_report(path: Path, result: LargeDenseComplexMeshCasesResult, shard_dir: Path) -> None:
    density = result.eval_workload.avg_candidates_per_query
    lines = [
        "# Three large complex dense mesh contact-pair STPF training and benchmark report",
        "",
        "## Objective",
        "",
        "- inondescriptionmulti-source dense benchmark descriptionon, description 3 description, description, descriptioncontactdescription case. ",
        "- descriptionselecthigh face-count ABC mesh, descriptionconstruct ABC description, ABC-Fusion, ABC-Thingi10K description. ",
        "- Protocol: RTSTPFExact descriptionuse learned STPF; descriptioncorrectnessdescription exact/fallback description, descriptionMetricsas `FN=0` and exact-work reduction. ",
        "",
        ...
        "",
        "## descriptionProtocol notes",
        "",
        "- `Exact work` isbyreal mesh face count calibrated dense candidate certificate workload. ",
        "- `Total ms` is Python runner  STPF description/descriptionoverhead, is not final C++/CUDA exact kernel wall time. ",
        "- descriptionreportdescriptionissamecandidatedescriptionunder `FN=0`, and exact work descriptionunderdescription. ",
        "",
        "## description",
        "",
    ...
    for metric in (result.combined_no_proposal, result.combined_trained_stpf):
        lines.append(
    ...
    lines.extend(
        [
            ...
            "## Conclusion",
            "",
            f"- added 3 description dense case, eval descriptioncandidatedescriptionas `{result.eval_workload.candidate_count}`, density as `{density:.0f}` candidates/query. ",
            f"- Learned STPF description exact work from `{_fmt(result.combined_no_proposal.exact_work_units, 1)}` reduced to `{_fmt(result.combined_trained_stpf.exact_work_units, 1)}`, reduction `{_pct(_reduction(result.combined_trained_stpf, result.combined_no_proposal))}`. ",
            f"- Correctness: trained RTSTPFExact `FN={result.combined_trained_stpf.fn_count}`. ",
            "- description 3  case descriptionondescription, high candidate densityanddescriptiongeneralization, descriptionasthis paper dense/high-cost workload description. ",
            "",
        ]
    )
